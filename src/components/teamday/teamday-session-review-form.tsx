...
import type { TeamdaySessionRecord } from "@/lib/teamday-sessions";
...
type TeamdaySessionReviewFormProps = {
  program: TeamdayProgram;
  session: TeamdaySessionRecord;
};

export function TeamdaySessionReviewForm({ program, session }: TeamdaySessionReviewFormProps) {
  ...
  const sessionsWithRatings = useMemo(() => {
    return program.sessions.filter((s) => sessionsState[s.id]?.rating);
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const selectedEntries = program.sessions
      .map((s) => {
        const entry = sessionsState[s.id];
        if (!entry?.rating) {
          return null;
        }
        return {
    ...
    const finalName = isAnonymous ? "Anoniem" : reviewerName.trim();

    if (!isAnonymous && !finalName) {
      setMessage({ type: "error", text: "Vul je naam in of kies voor anonieme review." });
      return;
    }
    ...
    try {
      const response = await fetch("/api/teamday/reviews", {
        ...
        body: JSON.stringify({
          reviewerName: finalName || "Anoniem",
      ...
      setMessage({
        type: "success",
        text: "Bedankt voor je feedback! Je review is opgeslagen en wordt gepubliceerd na controle.",
      });
    } catch (error: any) {
      ...
      <section className="rounded-3xl border border-neutral-200 bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h2 className="text-2xl font-semibold text-neutral-900">Review van deze teamdag</h2>
            <p className="mt-2 text-sm text-neutral-600">
              Geef per onderdeel aan hoe je de sessie hebt ervaren. Je naam is optioneel - je mag ook anoniem feedback geven.
            </p>
          </div>
        ...
        <div className="mt-6 space-y-4">
          <label className="flex items-center gap-2 text-sm">
            ...
            />
            <span className="font-medium text-neutral-700">Anonieme review</span>
          </label>
          ...
          return (
            ...
            >
                  ...
                  {[1, 2, 3, 4, 5].map((value) => {
                    ...
                    return (
                      <button
                        key={value}
                        type="button"
                        onClick={() => handleRatingChange(s.id, rating === value ? null : value)}
              ...
              <label className="block text-sm font-medium text-neutral-700">
                ...
                <textarea
                  value={comment}
                  onChange={(event) => handleCommentChange(s.id, event.target.value)}
                  rows={3}
